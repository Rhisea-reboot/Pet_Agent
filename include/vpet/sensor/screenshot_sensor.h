 ...
 * @brief 自动截图传感器
 *
 * 使用 QTimer 定时抓取屏幕，并将最新帧编码为 Base64，供后续聊天 Agent 使用。
 */
...
public:
    ...
    /**
     * @brief 获取最新截图的原始图像字节
     * @return 原始图像字节；无有效帧时返回空数组
     */
    QByteArray GetLatestFrameBytes() const;

    /**
     * @brief 获取最新截图的 Base64 数据
     * @return Base64 数据；无有效帧时返回空数组
     */
    QByteArray GetLatestFrameBase64() const;
...
signals:
     ...
     */
    void FrameCaptured(const QByteArray &base64Data, int frameCount, const QSize &frameSize);
...
private:
    ...
    QByteArray m_latestFrameBytes;   ///< 最新截图原始图像字节
    QByteArray m_latestFrameBase64;  ///< 最新截图 Base64 数据

...
bool ScreenshotSensor::CaptureOnce()
{
    ...
    const QByteArray frameBytes = VisionEncoder::ToImageBytes(pixmap,
                                                              m_config.imageFormat,
                                                              m_config.quality);

    if (frameBytes.isEmpty())
    {
        emit ErrorOccurred(QStringLiteral("Failed to encode screenshot bytes."));
        return false;
    }

    const QByteArray frameBase64 = frameBytes.toBase64();

    if (frameBase64.isEmpty())
    {
        emit ErrorOccurred(QStringLiteral("Failed to encode screenshot as Base64."));
        return false;
    }

    m_latestFrame = pixmap;
    m_latestFrameBytes = frameBytes;
    m_latestFrameBase64 = frameBase64;
    m_frameCount += 1;
    m_latestFilePath = SaveFrameToDisk(pixmap);

    emit FrameCaptured(m_latestFrameBase64, m_frameCount, m_latestFrame.size());
...
QByteArray ScreenshotSensor::GetLatestFrameBytes() const
{
    return m_latestFrameBytes;
}

QByteArray ScreenshotSensor::GetLatestFrameBase64() const
{
    return m_latestFrameBase64;
}

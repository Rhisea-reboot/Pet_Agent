                                                ...
                                                QString &errorMessage)
{
    ...
    if (terminalResult.local.GetValue(AgentContextKeys::CONVERSATION_HISTORY, historyValue)
        && !sessionContext.SetValue(AgentContextKeys::CONVERSATION_HISTORY, historyValue))
    {
    ...
    {
        sessionContext.RemoveValue(AgentContextKeys::CONVERSATION_HISTORY);
    }
    ...
    for (const QString &key : persistentKeys)
    {
        ...
        if (terminalResult.local.GetValue(key, value) && !sessionContext.SetValue(key, value))
        {
    ...
    return BuildExecutionView(terminalNodeId, sessionContext, context, errorMessage);
}
